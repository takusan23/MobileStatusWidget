    ...
    companion object {
        ...
        fun updateAppWidget(context: Context) {
            ...
            ids.forEach { id ->
                ...
                val pendingIntent = PendingIntent.getBroadcast(context, 1, Intent(context, MobileDataUsageWidget::class.java), PendingIntent.FLAG_UPDATE_CURRENT)
                ...
                if (MobileDataUsageTool.isGrantedUsageStatusPermission(context)) {
                    ...
                    views.setTextViewText(
                        R.id.widget_mobile_data_usage_text_view, """
                    $usageGB GB
                    ($usageMB MB)
                """.trimIndent()
                    )
